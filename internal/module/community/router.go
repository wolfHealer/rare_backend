...
func Register(r *gin.RouterGroup) {
	...
	post.PUT("/posts/:id", UpdatePost)         // 更新帖子
	post.DELETE("/posts/:id", DeletePost)      // 删除帖子
	...
	post.PUT("/comments/:id", UpdateComment)         // 更新评论
	post.DELETE("/comments/:id", DeleteComment)      // 删除评论

	comments := post.Group("/comments")
	comments.GET("/tree", GetCommentTree)
	comments.GET("/replies", GetCommentReplies)
